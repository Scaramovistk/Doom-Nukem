...
static bool	sector_panel_click(t_editor *e, int x, int y)
{
	...
	if (y >= 332 && y < 356 && e->selected_x >= 0)
		return (editor_project_key(KEY_TAB, e), true);
	if (y >= 356 && y < 380)
...
static bool	action_panel_click(t_editor *e, int x, int y)
{
	...
	if (y >= 502 && y < 530)
		return (editor_project_key(KEY_HOME, e), true);
	if (y >= 530 && y < 558)
		return (editor_project_key(KEY_END, e), true);
	if (y >= 558 && y < 586)
		return (editor_project_key(KEY_DELETE, e), true);
	return (false);
}
