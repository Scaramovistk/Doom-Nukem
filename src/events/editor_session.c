...
static void	add_action(t_editor_doc *doc, char *command)
{
	char	line[LINE_SIZE];

	if (snprintf(line, sizeof(line), "ACTION %s", command + 11)
		>= (int)sizeof(line) || !authored_action_line_valid(line))
	{
...
static void	clear_actions(t_editor_doc *doc)
{
	int	i;
