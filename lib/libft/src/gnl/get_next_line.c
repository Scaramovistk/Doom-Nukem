...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/10/15 18:54:17 by gscarama          #+#    #+#             */
/*   Updated: 2024/06/27 18:23:26 by rperez-t         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*extract_line(char *line, char *stash, int *eol_loc, int fd)
{
	...
	while (*eol_loc == -1)
	{
		...
		{
			free(line);
			ft_bezero_gnl(stash, (BUFFER_SIZE + 1));
			return (NULL);
		}
...
{
	static char	stash[FDS_TO_OPEN][BUFFER_SIZE + 1];
	char		*line;
	int			eol_loc;

	if (cleaner == true)
		return (ft_bezero_gnl(stash[fd], BUFFER_SIZE + 1), NULL);
	if (fd < 0 || fd >= FDS_TO_OPEN || BUFFER_SIZE <= 0)
		return (NULL);
	eol_loc = -1;
